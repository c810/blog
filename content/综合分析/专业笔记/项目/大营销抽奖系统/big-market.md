...
1. 如果在前面章节所实现的规则树中，对于库存节点的操作，开发 decr 方式扣减库存。decr 是原子操作，效率非常高。这样要注意，setnx 加锁是一种兜底手段，避免后续有库存的恢复，导致库存从 96 消耗后又回到了 98 重复消费。所以对于每个 key 加锁，98、97、96… 即使有恢复库存也不会导致超卖。【setnx 在 redisson 是用 trySet 实现】（TODO）
