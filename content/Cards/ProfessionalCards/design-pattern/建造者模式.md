...
将一个复杂对象的构建（零件）与表示（成品）分离（解耦），使得同样的构建过程（零件组装过程）可以创建不同的表示。

简单来说就是：**将自变量和因变量解耦，f 不变，自变量变，导致因变量变**。比如下例，只要组装的过程不变（都是先 * 后 +），那么里面的零件换了，也可以组装成成品，但由于零件已经换了，所以成品肯定不同。
...
首先一目了然，其次将构建的顺序交给了客户。

#TODO 老师没讲完整，这个适用于有必选参数和可选参数且参数过多时适用？
